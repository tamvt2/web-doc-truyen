...
if (!function_exists('convert_vi_to_en')) {
    function convert_vi_to_en($str) {
        ...
        $str = str_replace($vietnameseChars, $latinChars, $str);
        $str = strtolower($str);
        $str = preg_replace('/[^a-z0-9-\s]/', '', $str); // Remove special characters
        $str = preg_replace('/([\s]+)/', '-', $str); // Replace spaces with dashes

        return $str;
    }
}

if (!function_exists('kebabToTitleCase')) {
    function kebabToTitleCase($string) {
        // Thay thế dấu gạch ngang bằng khoảng trắng
        $string = str_replace('-', ' ', $string);

        // Chuyển đổi chuỗi thành dạng tiêu đề với hỗ trợ Unicode
